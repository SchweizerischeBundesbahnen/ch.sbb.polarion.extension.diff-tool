 ...
 * compile-time constant). Visual suites gate themselves on it with
 * `describe.skipIf(!__PIXEL_REFERENCES__)` so a run on any other host reports the behaviour results
 * instead of failing on the host's font metrics.
 */
