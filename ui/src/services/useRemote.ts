...
/**
 * Module-level and therefore reference-stable, which matters: react-sbb-polarion's About and
 * RevisionsTable list the injected request function in their effect dependencies, so a fresh identity
 * per render would refetch on every state change of the page hosting them. Being hook-free also keeps
...
const remote: { sendRequest: SendRequest } = { sendRequest: sendRequest };
