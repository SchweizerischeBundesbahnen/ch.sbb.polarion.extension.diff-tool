 ...
 * `documentation` node; About is its own node. Everything else opens no node (a settings page no `.html`
 * doc link targets). Built by the shared RSP helper from the doc manifest.
 */
export const nodeForFeature = docNodeForFeature({ docs: DOC_ORDER, selfNodes: ['about'] });
...
const adminNav = createAdminNav({ adminBase: 'diff-tool', nodeForFeature });
